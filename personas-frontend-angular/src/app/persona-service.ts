...
export class PersonaService {
  ...
  agregarPersona(persona: Persona) {
    ...
    this.dataService.agregarPersona(persona).subscribe((persona: Persona) => {
      ...
      console.log(
        'se agregar al arreglo la persona recien Insertada subscribe: ' +
          persona.idPersona
      );
  ...
  encontrarPersona(id: number) {
    const persona: Persona = this.personas.find(
      (persona) => persona.idPersona == id
    );
    console.log(
      'Persona Encontrada: ' + persona.idPersona + ' ' + persona.nombre
    );
    return persona;
  }
  ...
  modificarPersona(id: number, persona: Persona) {
    ...
    const personaModificadaLocal = this.personas.find(
      (persona) => persona.idPersona == id
    );
